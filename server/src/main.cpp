...
int rsf_initialize_bluetooth(void);
int rsf_finalize_bluetooth_socket(void);
int rsf_finalize_bluetooth(void);
int rsf_listen_connection(void);
int rsf_send_data(const char *, int);
void rsf_main_loop(void);

// Callbacks
void rsf_received_data_cb(bt_socket_received_data_s *, void *);
void rsf_socket_connection_state_changed_cb(int, bt_socket_connection_state_e, bt_socket_connection_s *, void *);
void rsf_state_changed_cb(int, bt_adapter_state_e, void *);
gboolean timeout_func_cb(gpointer);

int rsf_initialize_bluetooth(const char *device_name) {
	...
	{
		if(bt_adapter_set_state_changed_cb(rsf_state_changed_cb, NULL) != BT_ERROR_NONE)
		{
	...
	ret = bt_socket_set_connection_state_changed_cb(rsf_socket_connection_state_changed_cb, NULL);
	if(ret != BT_ERROR_NONE) {
		ALOGD("Unknown exception is occured in bt_socket_set_connection_state_changed_cb(): %x", ret);
		return -9;
	}

	ret = bt_socket_set_data_received_cb(rsf_received_data_cb, NULL);
	if(ret != BT_ERROR_NONE) {
...
int rsf_send_data(const char *data, int length) {
	int ret = bt_socket_send_data(gSocketFd, data, length);
	if(ret != BT_ERROR_NONE) {
		ALOGD("RemoteKeyFW: unknown error is occured in rsf_serror_end_data()");
		return -1;
	} else {
		return 0;
	}
...
void rsf_received_data_cb(bt_socket_received_data_s *data, void *user_data) {
	...
	//rsf_send_data(ack_string, strlen(ack_string)+1);
}

// bt_socket_connection_state_changed_cb
void rsf_socket_connection_state_changed_cb(int result, bt_socket_connection_state_e connection_state_event, bt_socket_connection_s *connection, void *user_data) {
...
void rsf_state_changed_cb(int result, bt_adapter_state_e adapter_state, void *user_data) {
...
int main(int argc, char *argv[])
{
	...
	error = rsf_initialize_bluetooth(device_name);
	if(error != 0) {
		ret = -2;
		goto error_end_without_socket;
	}
	ALOGD("succeed in rsf_initialize_bluetooth()\n");

	// Listen connection
	error = rsf_listen_connection();
	if(error != 0) {
		ret = -3;
...
error_end_with_socket:
	// Finalized bluetooth
	rsf_finalize_bluetooth_socket();

error_end_without_socket:
	rsf_finalize_bluetooth();
	return ret;
}
